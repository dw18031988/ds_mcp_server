...
mode: tighten_only
source_note: Includes the approved Rental Home Kiro spec-driven format and architecture
  constraints.
---

# Rental Home Project Extension

> This extension adds project-specific conventions. It cannot weaken the Generic Coding Project Governance.

## Mandatory project context

- `AGENTS.md` is the project source of truth when present on the protected base.
- Use one dedicated branch, worktree, and session folder per task.
- Read the configured `.ai` router, loader, config, and relevant skillset before applying a skill.
- Do not call Supabase directly from UI components; use the repository/dependency abstractions defined by the project.
- Preserve RLS, role, ownership, audit, and data-isolation behavior.
- Root `TASK.md` and the project task board must be updated when required by protected-base governance.
- Do not implement code merely because a spec was requested; implementation requires the applicable execution approval.

## Kiro spec-driven convention
